...
data:
  ...
  bundledCode: "#line 2 \"math/multipoint_evaluation.hpp\"\n#include <atcoder/internal_bit>\n\
    ...
    \n\ntemplate <class T, Mode mode>\nstd::vector<T> FormalPowerSeries<T, mode>::multipoint_evaluation(const\
  ...
  code: "#pragma once\n#include <atcoder/internal_bit>\n#include <vector>\n\n#include\
    ...
    \ FormalPowerSeries<T, mode>::multipoint_evaluation(const std::vector<T> &p) {\n\
    \tusing fps = FormalPowerSeries<T, mode>;\n\tint m = p.size();\n\tint n = 1 <<\
    \ atcoder::internal::ceil_pow2(m);\n\tstd::vector<fps> subproducts(2 * n, F{1}),\
    \ rem(2 * n);\n\tfor(int i = n; i < n + m; i++) subproducts[i] = fps({-p[i - n],\
    \ 1});\n\tfor(int i = n - 1; i; i--) {\n\t\tint x = subproducts[i << 1].size(),\
    \ y = subproducts[i << 1 | 1].size();\n\t\tsubproducts[i] = subproducts[i << 1];\n\
    \t\tsubproducts[i].resize(x + y - 1);\n\t\tsubproducts[i] *= subproducts[i <<\
    \ 1 | 1];\n\t}\n\trem[1] = *this;\n\tfor(int i = 1; i < n; i++) {\n\t\trem[i <<\
