...
data:
  ...
  bundledCode: "#line 2 \"graph/graph.hpp\"\n#include <algorithm>\n#include <cassert>\n\
    ...
    \ cycle.empty() and (to != par or multiple_par)) {\n\t\t\t\tfor(const auto &v\
  ...
  code: "#pragma once\n#include \"graph/graph.hpp\"\n\nstruct namori {\npublic:\n\t\
    ...
    \ continue;\n\t\t\tcycles.emplace_back();\n\t\t\tbuild_dfs(i, -1, cycles.back());\n\
    \t\t\tif(cycles.back().empty()) cycles.back().push_back(i);\n\t\t}\n\t\tfor(int\
    \ i = 0; i < (int)cycles.size(); i++)\n\t\t\tfor(const auto &v : cycles[i]) cycle_number[v]\
    \ = i;\n\t\tdfs_used.assign(n, false);\n\t\tfor(int i = 0; i < n; i++)\n\t\t\t\
    if(cycle_number[i] != -1) tree_dfs(i, i);\n\t}\n\n\tvoid build_dfs(int now, int\
    \ par, std::vector<int> &cycle) {\n\t\tdfs_used[now] = true;\n\t\tdfs_now[now]\
    \ = true;\n\t\tdfs_ver.push_back(now);\n\t\tbool multiple_par = false;\n\t\tfor(const\
    \ auto [to, cost] : g[now]) {\n\t\t\tif(dfs_now[to] and cycle.empty() and (to\
